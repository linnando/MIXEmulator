...
class ShiftSpec extends AsyncWordSpec with Matchers {
  ...
  "binary shift module" should {
    "shift A to the left" in {
      // A = 2, I = 0, F = 0, C = 6 SLA
      execute(state, MixWord(0x00080006)).map(s => {
    ...
    "shift A to the right" in {
      // A = 2, I = 0, F = 1, C = 6 SRA
      execute(state, MixWord(0x00080046)).map(s => {
    ...
    "shift AX to the left" in {
      ...
      execute(state, MixWord(0x00040086)).map(s => {
    ...
    "shift AX to the right" in {
      ...
      execute(state, MixWord(0x000400c6)).map(s => {
    ...
    "shift AX to the left circularly" in {
      // A = 1, I = 0, F = 4, C = 6 SLC
      execute(state, MixWord(0x00040106)).map(s => {
        s.registers.getA mustEqual MixWord(0x020c4146) // + 2 3 4 5 6
    ...
    "shift AX to the right circularly" in {
      // A = 1, I = 0, F = 5, C = 6 SRC
      execute(state, MixWord(0x00040146)).map(s => {
