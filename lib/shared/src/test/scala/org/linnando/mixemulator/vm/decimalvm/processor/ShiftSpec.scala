...
class ShiftSpec extends AsyncWordSpec with Matchers {
  ...
  "decimal shift module" should {
    "shift A to the left" in {
      // A = 2, I = 0, F = 0, C = 6 SLA
      decimal.execute(state, MixWord(2000006L)).map(s => {
    ...
    "shift A to the right" in {
      // A = 2, I = 0, F = 1, C = 6 SRA
      decimal.execute(state, MixWord(2000106L)).map(s => {
    ...
    "shift AX to the left" in {
      ...
      decimal.execute(state, MixWord(1000206L)).map(s => {
    ...
    "shift AX to the right" in {
      ...
      decimal.execute(state, MixWord(1000306L)).map(s => {
    ...
    "shift AX to the left circularly" in {
      // A = 1, I = 0, F = 4, C = 6 SLC
      decimal.execute(state, MixWord(1000406L)).map(s => {
        s.registers.getA mustEqual MixWord(203040506L) // + 2 3 4 5 6
    ...
    "shift AX to the right circularly" in {
      // A = 1, I = 0, F = 5, C = 6 SRC
      decimal.execute(state, MixWord(1000506L)).map(s => {
