...
class BinaryIOSpec extends Specification {
  private val state = initialState.copy(
    registers = initialState.registers.updatedX(BinaryMixWord(2000)),
  ...
  "binary input" should {
    "call read for a positional input device" in {
      ...
      val nextState = execute(state, BinaryMixWord(0x0fa00024))
      val device = nextState.devices(0)
      nextState.memory.exclusiveLocks must contain((BinaryMixIndex(1000), device._1.blockSize, 0))
      device._1 match {
        case d: MockPositionalInputDevice => d.counter must beGreaterThan(0)
        case _ => throw new Error
      }
      device._2 must contain(BinaryMixIndex(1000))
    }

    "throw an exception for a positional output device" in {
      // A = 1000, I = 0, F = 1, C = 36 IN
      execute(state, BinaryMixWord(0x0fa00064)) must throwAn[UnsupportedOperationException]
    }

    "call read for a random access device" in {
      // A = 1000, I = 0, F = 2, C = 36 IN
      val nextState = execute(state, BinaryMixWord(0x0fa000a4))
      val device = nextState.devices(2)
      nextState.memory.exclusiveLocks must contain((BinaryMixIndex(1000), device._1.blockSize, 2))
      device._1 match {
  ...
  "binary output" should {
    "throw an exception for a positional input device" in {
      ...
      execute(state, BinaryMixWord(0x0fa00025)) must throwAn[UnsupportedOperationException]
    }

    "call write for a positional output device" in {
      // A = 1000, I = 0, F = 1, C = 37 OUT
      val nextState = execute(state, BinaryMixWord(0x0fa00065))
      val device = nextState.devices(1)
      nextState.memory.sharedLocks must contain((BinaryMixIndex(1000), device._1.blockSize, 1))
      device._1 match {
    ...
    "call write for a random access device" in {
      ...
      val nextState = execute(state, BinaryMixWord(0x0fa000a5))
      val device = nextState.devices(2)
      nextState.memory.sharedLocks must contain((BinaryMixIndex(1000), device._1.blockSize, 2))
      device._1 match {
  ...
  "binary input/output control" should {
    "throw an exception for a generic device" in {
      ...
      execute(state, BinaryMixWord(0x00000023)) must throwAn[UnsupportedOperationException]
      // A = 0, I = 0, F = 1, C = 35 IOC
      execute(state, BinaryMixWord(0x00000063)) must throwAn[UnsupportedOperationException]
      // A = 0, I = 0, F = 2, C = 35 IOC
      execute(state, BinaryMixWord(0x000000a3)) must throwAn[UnsupportedOperationException]
    }

    "position a tape unit" in {
      // A = 10, I = 0, F = 3, C = 35 IOC
      val nextState = execute(state, BinaryMixWord(0x002800e3))
    ...
    "position a disk unit" in {
      ...
      val nextState = execute(state, BinaryMixWord(0x00000123))
    ...
    "throw an exception if M != 0 for a disk unit" in {
      ...
      execute(state, BinaryMixWord(0x00040123)) must throwA[WrongMemoryAddressException]
    }

    "switch page on a line printer" in {
      // A = 0, I = 0, F = 5, C = 35 IOC
      val nextState = execute(state, BinaryMixWord(0x00000163))
    ...
    "throw an exception if M != 0 for a line printer" in {
      ...
      execute(state, BinaryMixWord(0x00040163)) must throwA[WrongMemoryAddressException]
    }

    "call reset for a paper tape" in {
      // A = 0, I = 0, F = 6, C = 35 IOC
      val nextState = execute(state, BinaryMixWord(0x000001a3))
    ...
    "throw an exception if M != 0 for a paper tape" in {
      ...
      execute(state, BinaryMixWord(0x000401a3)) must throwA[WrongMemoryAddressException]
    }
  }
  ...
  "device-conditional jump" should {
    "trigger jump ready and not trigger jump busy when device is not busy" in {
      ...
      val nextStateRed = execute(state, BinaryMixWord(0x0fa00026))
      nextStateRed.programCounter must be equalTo BinaryMixIndex(1000)
      nextStateRed.registers.getJ must be equalTo BinaryMixIndex(1)
      // A = 1000, I = 0, F = 0, C = 34 JBUS
      val nextStateBus = execute(state, BinaryMixWord(0x0fa00022))
      nextStateBus.programCounter must be equalTo BinaryMixIndex(1)
      nextStateBus.registers.getJ must be equalTo BinaryMixIndex(0)
    }

    "throw an exception on jump ready when device is busy" in {
      // A = 1000, I = 0, F = 0, C = 36 IN
      val prevState = execute(state, BinaryMixWord(0x0fa00024))
      // A = 1000, I = 0, F = 0, C = 38 JRED
      execute(prevState, BinaryMixWord(0x0fa00026)) must throwAn[UnpredictableExecutionFlowException]
    }
    ...
    "throw an exception on jump busy when device is busy and the address is not the same as the program counter" in {
      ...
      val prevState = execute(state, BinaryMixWord(0x0fa00024))
      // A = 1000, I = 0, F = 0, C = 34 JBUS
      execute(prevState, BinaryMixWord(0x0fa00022)) must throwAn[UnpredictableExecutionFlowException]
    }
    ...
    "flush device input on jump busy when device is reading and the address is the same as the program counter" in {
      ...
      val nextState = execute(prevState, BinaryMixWord(0x00040022))
      val device = nextState.devices(0)
      (0 until device._1.blockSize) forall { i =>
        nextState.memory.get(BinaryMixIndex((1000 + i).toShort)) must be equalTo BinaryMixWord(0x01041041)
      }
      nextState.memory.exclusiveLocks must not contain((BinaryMixIndex(1000), device._1.blockSize, 0))
      nextState.programCounter must be equalTo BinaryMixIndex(2)
