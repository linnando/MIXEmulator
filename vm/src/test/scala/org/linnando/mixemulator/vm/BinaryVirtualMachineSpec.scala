...
class BinaryVirtualMachineSpec extends Specification {
  "binary non-tracking virtual machine" should {
    "load initial state" in {
      val machine = BinaryProcessingModel.createVirtualMachineBuilder()
        ...
        .build
      machine.currentState.memory.get(BinaryProcessingModel.BinaryMixIndex(2000)) must
        be equalTo BinaryProcessingModel.BinaryMixWord(0x41403144)
      machine.currentState.memory.get(BinaryProcessingModel.BinaryMixIndex(3000)) must
        be equalTo BinaryProcessingModel.BinaryMixWord(0x1f400148)
      machine.currentState.programCounter must be equalTo BinaryProcessingModel.BinaryMixIndex(3000)
    }
    ...
    "go one step forward" in {
      val machine = BinaryProcessingModel.createVirtualMachineBuilder()
        ...
        .build
      machine.stepForward()
      machine.currentState.registers.getA must be equalTo BinaryProcessingModel.BinaryMixWord(0x41403144)
      machine.currentState.programCounter must be equalTo BinaryProcessingModel.BinaryMixIndex(3001)
    }
    ...
    "go to the next breakpoint" in {
      ...
      machine.runForward()
      machine.currentState.registers.getA must be equalTo BinaryProcessingModel.BinaryMixWord(0x41403144)
      machine.currentState.programCounter must be equalTo BinaryProcessingModel.BinaryMixIndex(3005)
    }
    ...
    "not execute the command at the breakpoint" in {
      ...
      machine.toggleBreakpoint(3005)
      machine.runForward()
      machine.currentState.registers.getA must be equalTo BinaryProcessingModel.BinaryMixWord(0x00000000)
      machine.currentState.programCounter must be equalTo BinaryProcessingModel.BinaryMixIndex(3005)
    }
    ...
    "stop when HLT is encountered before the next breakpoint" in {
      ...
      machine.toggleBreakpoint(3005)
      machine.runForward()
      machine.currentState.programCounter must be equalTo BinaryProcessingModel.BinaryMixIndex(3003)
      machine.currentState.isHalted must beTrue
    }
